...
- **Sharing Permissions & Authoritative Capability Matrix (SHARE-03 & SHARE-04):**
  ...
  - **Unique Sharing Integrity & Account Enumeration Defense:** The database enforces a composite unique constraint on `(note_id, shared_with_user_id)` preventing duplicate share grants. Self-sharing (`shared_with_user_id === user_id`) is strictly rejected. When attempting to share with an email address not present in the system, the API returns a generic `422 Unprocessable Entity` (`"Unable to share with that email address."`) to prevent account enumeration / user discovery.
  - **Recipient Shared Workspace Isolation (SHARE-05):** The `GET /api/shared-notes` endpoint scopes strictly to notes where the authenticated user has an active grant (`receivedShares`). Locked shared notes return `content: null` until unlocked. Owner organizational labels and owner password hashes are strictly excluded from `SharedNoteResource`. Recipients cannot perform owner-only operations (pin, delete, set password, manage shares).
