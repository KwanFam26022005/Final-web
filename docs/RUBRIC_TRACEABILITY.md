...
| **NOTE-08** | Phase 4 | MANDATORY COURSE REQUIREMENT | File Attachments | PHPUnit `AttachmentTest.php`, Vitest `NoteAttachments.test.tsx`, Playwright Test 16 | Permitted files upload (JPEG/PNG/WebP/PDF max 10MB), validate MIME/size, attach to note with download/delete | **VERIFIED** |
| **LABEL-01** | Phase 4 | MANDATORY COURSE REQUIREMENT | Label CRUD | PHPUnit `LabelTest.php`, Vitest `LabelManagement.test.tsx`, Playwright Test 15 | Create, view, edit/rename, and delete custom labels with note preservation | **VERIFIED** |
| **LABEL-02** | Phase 4 | MANDATORY COURSE REQUIREMENT | Many-to-Many Association | PHPUnit `LabelTest.php`, Vitest `NoteLabelAssignment.test.tsx`, Playwright Test 15 | Assign multiple labels per note and multiple notes per label via note_label pivot | **VERIFIED** |
| **LABEL-03** | Phase 4 | MANDATORY COURSE REQUIREMENT | Label Filtering | PHPUnit `LabelTest.php`, Vitest `NoteLabelFiltering.test.tsx`, Playwright Test 15 | Selecting labels filters notes list with ALL-match semantics and composite search | **VERIFIED** |
| **SHARE-01** | Phase 5 | MANDATORY COURSE REQUIREMENT | Per-Note Password Protection | PHPUnit `NoteProtectionTest.php`, Vitest `NoteProtection.test.tsx`, Playwright Test 17 | Locked note displays lock icon; content hidden until unlocked; session-isolated unlock | **VERIFIED** |
| **SHARE-02** | Phase 5 | MANDATORY COURSE REQUIREMENT | Server-Side Unlock Verification | PHPUnit `NoteProtectionTest.php`, Vitest `NoteProtection.test.tsx`, Playwright Test 17 | Submitting password validates on backend before returning note body; client never holds unverified secrets | **VERIFIED** |
| **SHARE-03** | Phase 5 | MANDATORY COURSE REQUIREMENT | User-to-User Note Sharing | PHPUnit `NoteSharingTest.php`, Vitest `NoteSharing.test.tsx`, Playwright Test 18 | Owner shares note with registered user by email address; unique share constraint enforced | **VERIFIED** |
| **SHARE-04** | Phase 5 | MANDATORY COURSE REQUIREMENT | Read / Edit Permissions | PHPUnit `NoteSharingTest.php`, Vitest `NoteSharing.test.tsx`, Playwright Test 18 | Server enforces `read` vs. `edit` permissions; owner controls (pin, delete, protection, labels, shares) restricted | **VERIFIED** |
| **SHARE-05** | Phase 5 | MANDATORY COURSE REQUIREMENT | Shared-Note Metadata Exposure | PHPUnit `NoteSharingTest.php`, Vitest `SharedWorkspace.test.tsx`, Playwright Test 19 | Recipient workspace renders permission badge, sharer display/email, share date, locked badge, grid/list view | **VERIFIED** |
| **SHARE-06** | Phase 5 | OPTIONAL / FUTURE ENHANCEMENT | Access Revocation | PHPUnit `NoteSharingTest.php`, Vitest `NoteSharing.test.tsx`, Playwright Test 18, 19 | Owner revokes collaborator access; recipient immediately loses access with 403 Forbidden | **VERIFIED** |
| **RT-01** | Phase 6 | MANDATORY COURSE REQUIREMENT | Realtime Collaborative Editing | PHPUnit `RealtimeNoteSyncTest.php`, Vitest `NoteRealtime.test.tsx`, `echo.test.ts`, Playwright Test 20 | Changes made in Browser A appear in Browser B in real time via Reverb without reload | **VERIFIED** |
...
| **SEC-04** | Phase 4 | MANDATORY COURSE REQUIREMENT | File Upload Security Validation | PHPUnit `AttachmentTest.php`, Vitest `NoteAttachments.test.tsx`, Playwright Test 16 | Dangerous extensions/MIMEs blocked; storage outside public web root; random UUID filenames; authorization required | **VERIFIED** |
