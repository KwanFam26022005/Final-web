...
| **NOTE-04** | Phase 3 | MANDATORY COURSE REQUIREMENT | Debounced Autosave | Vitest `useAutosave.test.ts` | Edits save automatically on pause with clear visual status indicator | PLANNED |
| **NOTE-05** | Phase 3 | MANDATORY COURSE REQUIREMENT | Safe Deletion Confirmation | Vitest `DeleteConfirmModal.test.tsx` | Confirmation prompt appears; deletion executes ONLY after confirmation | PLANNED |
| **NOTE-06** | Phase 4 | MANDATORY COURSE REQUIREMENT | Note Pinning & Priority Section | `PinNoteTest` & UI rendering test | User toggles pin; pinned notes render in top section with pin badge | PLANNED |
| **NOTE-07** | Phase 4 | MANDATORY COURSE REQUIREMENT | Live Instant Search | Vitest `SearchNotes.test.tsx` | Live debounced search on title and content without full-page reloads | PLANNED |
